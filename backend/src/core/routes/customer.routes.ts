    ...
    app.route("/customers")
        .get((req, res, next) => {
            customerRepo.find({}, (err, docs) => {
                ...
                res.format({
                    "application/hal+json": () =>  res.json(resourceUtils.createResources(req, config.urls.customers, docs)),
    ...
    app.route("/customers/:id")
        .get((req, res, next) => {
            customerRepo.findOne({id: req.params.id}, (err, doc) => {
                ...
                res.format({
                    "application/hal+json": () =>  res.json(resourceUtils.createResource(req, config.urls.customers, doc)),
            ...
            cmdSrv.send("changeCustomer").for("customer").instance(req.params.id).with({payload: req.body}).go(evt => {
                if (evt.event === "commandRejected") {
                    ...
                    res.format({
                        "application/hal+json": () => res.json(resourceUtils.createResource(req, config.urls.customers, evt.payload)),
